/**
 * src/components/Manager/matchSubComponents/doubleElim/MatchDoubleElim.tsx
 *
 * === Double Elimination (Create + Edit) ===
 * Orchestrates generation + display of a double-elimination bracket for an Event.
 * Reuses the single-elim fighter list/manager (format-agnostic).
 */
...
export interface BracketMatch {
  ...
  bracketNo: number;
  bracketSection: "W" | "L";
  matchRole?: "final" | "bronze" | "grandFinal" | "grandFinalReset" | null;
...
interface MatchDoubleElimProps {
  eventId: number;
  eventName: string;
  maxRings: number;
  isActive: boolean;
...
const MatchDoubleElim: React.FC<MatchDoubleElimProps> = ({
  eventId,
  eventName,
  maxRings,
  isActive,
  ...
  const [format, setFormat] = useState<"S" | "D" | undefined>("D");
  const [bracketId, setBracketId] = useState<number | undefined>(undefined);
  const [hasGrandFinalReset, setHasGrandFinalReset] = useState<boolean | undefined>(undefined);
  const [locallyActive, setLocallyActive] = useState<boolean>(false);

  const [activeMenu, setActiveMenu] = useState<string>(() => {
    return localStorage.getItem(`delimMenu-${eventId}`) || "editor";
  });

  useEffect(() => {
    localStorage.setItem(`delimMenu-${eventId}`, activeMenu);
  }, [activeMenu, eventId]);
  ...
  const fetchBracket = useCallback(async () => {
    ...
    try {
      ...
      if (typeof data.hasGrandFinalReset === "boolean")
        setHasGrandFinalReset(data.hasGrandFinalReset);
    } catch (err: any) {
  ...
  useEffect(() => {
    if (isActive) {
      ...
      setHasGrandFinalReset(undefined);
      setError(null);
    }
  ...
  const handleCreated = useCallback(
    (payload: FetchResponse) => {
      if (payload.status === "success") {
        ...
        if (typeof payload.bracketId === "number")
          setBracketId(payload.bracketId);
        setFormat(normalizeBracketFormat(payload.format));
        setHasGrandFinalReset(payload.hasGrandFinalReset);
        setError(null);
        setLocallyActive(true);
        addToast("Double-elimination bracket created.");
  ...
  return (
    <ErrorBoundary>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        >
          <h2 style={{ margin: 0 }}>Double Elimination — {eventName}</h2>
        </div>
        ...
        <div style={{ fontSize: 12, opacity: 0.65 }}>
          ...
          {typeof hasGrandFinalReset === "boolean"
            ? ` • Reset: ${hasGrandFinalReset ? "Yes" : "No"}`
            : ""}
        </div>
      </div>
    </ErrorBoundary>
...
export default MatchDoubleElim;