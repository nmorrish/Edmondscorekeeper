<?php
/**
 * phpFiles/viewerStandingsByPool.php
 *
 * Public API – Returns per-fighter standings grouped by Pool.
 * Uses PoolMatches + Pools join.
 */
...
try {
    $stmt = $db->prepare("
        SELECT 
            ...
            (
                ...
                INNER JOIN PoolMatches pm2 ON pm2.MatchId = m2.MatchId
                INNER JOIN Pools p2 ON pm2.PoolId = p2.PoolId
                WHERE mf2.FighterId = f.FighterId
                  AND m2.EventId = :eventId
                  AND m2.PendingActiveDone = 'D'
                  AND p2.PoolNo = p.PoolNo
            ) AS totalExchanges
        ...
        GROUP BY p.PoolNo, f.FighterId, f.FighterName, c.ClubName
        ORDER BY p.PoolNo ASC, wins DESC, name ASC
    ");
    ...
    foreach ($rows as &$r) {
        ...
        $r['PoolNo']         = isset($r['PoolNo']) ? (int)$r['PoolNo'] : null;
